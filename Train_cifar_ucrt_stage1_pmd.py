...
import dataloader_cifar_idn_pmd as dataloader
import pdb
import io
import PIL
...
parser.add_argument('--noise_mode', default='1')
...
parser.add_argument('--r', default=0.35, type=float, help='noise ratio')
...
parser.add_argument('--cdn_mode', default='U', type=str, help='additional cdn noise mode, default is None, A or U')
parser.add_argument('--cdn_rate', default=0.3, type=float, help='additional cdn noise rates, default is 0., 0.3 or 0.6')
...
def train(epoch, net, net2, optimizer, labeled_trainloader, unlabeled_trainloader, savelog=False):
    ...
    for batch_idx, (inputs_x, inputs_x2, inputs_x3, inputs_x4, labels_x, w_x) in enumerate(labeled_trainloader):
        ...
        loss = Lx + lamb * Lu + penalty+ args.lambda_c*loss_simCLR
        ...
        sys.stdout.write('\r')
        sys.stdout.write(
            '%s:%.1f-%s | Epoch [%3d/%3d] Iter[%3d/%3d]\t Labeled loss: %.2f  Unlabeled loss: %.2f, cl loss: %.2f'
...
def Calculate_JSD(model1, model2):
    ...
    for batch_idx, (inputs, targets, index) in enumerate(eval_loader):
        ...
        with torch.no_grad():
            out1 = torch.nn.Softmax(dim=1).cuda()(model1(inputs, second_ind)[1])
            out2 = torch.nn.Softmax(dim=1).cuda()(model2(inputs, second_ind)[1])
        ...
        # 因为batch_size=100, shuffle=False,所以直接按照batch保存即可
    ...
    sample_ratio = torch.sum(prob < threshold).item() / len(eval_loader.dataset)
    pred_idx = np.zeros(len(eval_loader.dataset)).astype(np.bool)
    class_len = int(sample_ratio * len(eval_loader.dataset) / args.num_class)
    size_pred = 0
    ...
    for i in range(args.num_class):
        ...
        prob1 = np.argsort(prob[class_indices].cpu().numpy())
        size1 = len(class_indices)

        class_len_temp = min(size1, class_len)
        select_idx = class_indices[prob1[:class_len_temp]]
        pred_idx[select_idx] = True

    # prob = np.zeros(len(JSD))
    prob[prob<0.5]=0
    idx_labeled = (pred_idx).nonzero()[0]
...
def save_models(epoch, net1, optimizer1, net2, optimizer2, save_path):
    ...
    if epoch % 1 == 0:
        ...
        fn3 = os.path.join(save_path, 'hcs_%s_%.2f_%s_cn%d_run%d.pth.tar' % (
...
if __name__ == '__main__':
    ...
    if args.cdn_mode is None or args.cdn_rate == 0.:
        loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode,
                                             ...
                                             noise_file='noise/plc/%s-%s-%.2f.json' % (
                                             args.dataset, args.noise_mode, args.r))
    else:
        loader = dataloader.cifar_dataloader(args.dataset, r=args.r, noise_mode=args.noise_mode,
                                             batch_size=args.batch_size,
                                             num_workers=0,
                                             root_dir=args.data_path, log=stats_log,
                                             noise_file='noise/plc/%s-%s-%.2f_%s_%.1f.json' % (
                                                 args.dataset, args.noise_mode, args.r, args.cdn_mode, args.cdn_rate))
    ...
    if incomplete == True:
        ...
        superclean_path = os.path.join(path_exp, 'hcs_%s_%.2f_%s_cn%d_run%d.pth.tar' % (
        args.dataset, args.r, args.noise_mode, args.num_clean, args.run))
    ...
    for epoch in range(resume_epoch+1, args.num_epochs + 1):
        ...
        if epoch < warm_up:
            ...
            # if epoch % 5 == 0:
            #     # plot_graphs(epoch)
            #     plot_histogram_loss_pred(data=all_loss[0][-1].numpy(), inds_clean=inds_clean,
            ...
            # warm up阶段同样的
            prob1, pred1 = Calculate_JSD(net2, net1)
            ...
            print('Train Net1')
            labeled_trainloader, unlabeled_trainloader, _ = loader.run('train', pred1, prob1, second_ind=second_ind)  # co-divide
            train(epoch, net1, net2, optimizer1, labeled_trainloader, unlabeled_trainloader, savelog=True)  # train net1

            prob2, pred2 = Calculate_JSD(net2, net1)
            ...
            print('\nTrain Net2')
            labeled_trainloader, unlabeled_trainloader, u_map_trainloader = loader.run('train', pred2, prob2, second_ind=second_ind)  # co-divide
            ...
            #     inds_guess_wrong = np.asarray([idx_unlabeled[ind] for ind in range(len(idx_unlabeled))
            #                                    if clean_labels[idx_unlabeled[ind]] != guessed[ind]])
            #
            #     inds_guess_correct = np.asarray([idx_unlabeled[ind] for ind in range(len(idx_unlabeled))
            #                                      if clean_labels[idx_unlabeled[ind]] == guessed[ind]])
            #
            #     plot_guess_view(data=all_loss[0][-1].numpy(), inds_guess_correct=inds_guess_correct,
