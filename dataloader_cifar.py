...
class cifar_dataset(Dataset): 
    def __init__(self, dataset, r, noise_mode, root_dir, transform, mode, noise_file='', pred=[], probability=[], log='', second_ind=False):
        ...
        if self.mode=='test':
            ...
            if os.path.exists(noise_file):
                ...
                if dataset == 'cifar100' and noise_mode=='asym':
                    ...
                    print("incorrect num:%.2f"%(np.sum(np.array(train_label)!=np.array(noise_label))/50000.))
                else:
    ...
    def run(self,mode,pred=[],prob=[], second_ind = False):
        if mode=='warmup':
            ...
            labeled_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r,
                                            ...
                                            noise_file=self.noise_file, pred=pred, probability=prob,log=self.log, second_ind=second_ind)
            ...
            test_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r,
                                         root_dir=self.root_dir, transform=self.transform_test, mode='test', second_ind=second_ind)
            ...
            eval_dataset = cifar_dataset(dataset=self.dataset, noise_mode=self.noise_mode, r=self.r, root_dir=self.root_dir,
                                         transform=self.transform_test, mode='all', noise_file=self.noise_file, second_ind=second_ind)
